...
export const fetchOrderData = async (
  ...
  const url = `${API_URL}action=${GET_DATA_ACTION}&email=${email}`;
  try {
...
export const updateOrderStatus = async (
  ...
  const url = `${API_URL}action=${UPDATE_READY_ACTION}&id=${id}&listo=${status}`;
...
export const updateDeliveryDate = async (
  ...
  date = date ?? "";
  const url = `${API_URL}action=${UPDATE_DELIVERY_ACTION}&id=${id}&date=${date}`;
